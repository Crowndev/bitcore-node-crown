 ...
 * @param {String} protocolId - NFT protocol ID.
 * @param {String} ownerAddress - NFT owner address.
 * @param {String} count - The number of records to return.
 * @param {Function} callback
 */
Bitcoin.prototype.nftList = function(protocolId, ownerAddress, count, callback) {
  var self = this;
  this.client.nfToken("list", protocolId, ownerAddress, count, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
