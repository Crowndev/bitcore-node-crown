...
Bitcoin.DEFAULT_INSTANTSEND_FEE = 1000000;
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['nftBalanceOf', this, this.nftBalanceOf, 2],
    ['nftOwnerOf', this, this.nftOwnerOf, 2],
    ['nftGetByTxid', this, this.nftGetByTxid, 1],
    ['nftGet', this, this.nftGet, 2],
    ['nftList', this, this.nftList, 6],
    ['nftRegister', this, this.nftRegister, 5],
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  ...
  $.checkState(
    ...
    '"zmqpubrawtxlock" option is required to get transaction locks from bitcoind. ' +
...
Bitcoin.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  function addInputsToTx(tx, result) {
    ...
    for(var inputIndex = 0; inputIndex < result.vin.length; inputIndex++) {
      ...
      } else if (input.coinstake) {
        script = input.coinstake;
      }
      tx.inputs.push({
  ...
  function addTxlockToTx(tx,result) {
    tx.txlock = false;
    if (tx.feeSatoshis >= Bitcoin.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= DEFAULT, check for txlock
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        if (result.vin[0] && result.vin[0].coinstake) {
          tx.coinstake = true;
          tx.pointerHash = result.vin[0].pointer_hash;
        }
...
Bitcoin.prototype.nftTotalSupply = function(protocolId, callback) {
  ...
  if (protocolId === undefined) {
    this.client.nfToken("totalsupply", function(err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
      callback(null, response.result);
...
/**
 * Will get balance of NFTs belonging to a certain address within a protocol set or in a global set.
 * @param {String} ownerAddress - NFT owner address.
 * @param {String} protocolId - NFT protocol ID.
 * @param {Function} callback
 */
Bitcoin.prototype.nftBalanceOf = function(ownerAddress, protocolId, callback) {
  var self = this;
  if (protocolId === undefined) {
    this.client.nfToken("balanceof", ownerAddress, function(err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
      callback(null, response.result);
    ...
    this.client.nfToken("balanceof", ownerAddress, protocolId, function(err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
      callback(null, response.result);
...
/**
 * Will get address of the NFT owner by using its protocol ID and token ID.
 * @param {String} protocolId - NFT protocol ID.
 * @param {String} tokenId - Token ID.
 * @param {Function} callback
 */
Bitcoin.prototype.nftOwnerOf = function(protocolId, tokenId, callback) {
  var self = this;
  this.client.nfToken("ownerof", protocolId, tokenId, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
/**
 * Will get an nftoken record by its registration transaction ID.
 * @param {String} txid - NFT registration transaction ID.
 * @param {Function} callback
 */
Bitcoin.prototype.nftGetByTxid = function(txid, callback) {
  var self = this;
  this.client.nfToken("getbytxid", txid, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
/**
 * Will get an nftoken record by an NFT protocol ID and token ID.
 * @param {String} protocolId - NFT protocol ID.
 * @param {String} tokenId - Token ID.
 * @param {Function} callback
 */
Bitcoin.prototype.nftGet = function(protocolId, tokenId, callback) {
  var self = this;
  this.client.nfToken("get", protocolId, tokenId, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
/**
 * Will list nftoken records on chain.
 * @param {String} protocolId - NFT protocol ID.
 * @param {String} ownerAddress - NFT owner address.
 * @param {Number} count - The number of records to return.
 * @param {Number} skipFromTip - The number of transactions to skip from tip.
 * @param {Number} height - If height is not specified, it defaults to the current chain-tip.
 * @param {Boolean} regTxOnly - False for a detailed list, true for an array of transaction IDs.
 * @param {Function} callback
 */
Bitcoin.prototype.nftList = function(protocolId, ownerAddress, count, skipFromTip, height, regTxOnly, callback) {
  var self = this;

  if (protocolId === undefined) protocolId = '*';
  if (ownerAddress === undefined) ownerAddress = '*';
  if (count === undefined) count = 20;
  if (skipFromTip === undefined) skipFromTip = 0;
  if (height === undefined) height = '*';
  if (regTxOnly === undefined) regTxOnly = false;

  count = count.toString();
  skipFromTip = skipFromTip.toString();
  height = height.toString();
  regTxOnly = regTxOnly.toString();

  this.client.nfToken("list", protocolId, ownerAddress, count, skipFromTip, height, regTxOnly, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
/**
 * Will create and send a new non-fungible token transaction.
 * @param {String} protocolId - NFT protocol ID.
 * @param {String} tokenId - Token ID.
 * @param {String} ownerAddress - NFT owner address.
 * @param {String} metadataAdminAddress - The metadata token administration address, can be used to modify token metadata.
 * @param {String} metadata - Metadata describing the token.
 * @param {Function} callback
 */
Bitcoin.prototype.nftRegister = function(protocolId, tokenId, ownerAddress, metadataAdminAddress, metadata, callback) {
  var self = this;

  if (metadataAdminAddress === undefined) metadataAdminAddress = '0';
  if (metadata === undefined) metadata = '';

  this.client.nfToken("register", protocolId, tokenId, ownerAddress, metadataAdminAddress, metadata, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
/**
 * Will get address of the NFT protocol owner by using its protocol ID.
 * @param {String} protocolId - NFT protocol ID.
 * @param {Function} callback
 */
Bitcoin.prototype.nftProtocolOwnerOf = function(protocolId, callback) {
  var self = this;
  this.client.nftProto("ownerof", protocolId, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
/**
 * Will get an NFT protocol record by a transaction ID.
 * @param {String} txid - The NFT protocol registration transaction ID.
 * @param {Function} callback
 */
Bitcoin.prototype.nftProtocolGetByTxid = function(txid, callback) {
  var self = this;
  this.client.nftProto("getbytxid", txid, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
/**
 * Will get an NFT protocol record by an NFT protocol ID.
 * @param {String} protocolId - NFT protocol ID.
 * @param {Function} callback
 */
Bitcoin.prototype.nftProtocolGet = function(protocolId, callback) {
  var self = this;
  this.client.nftProto("get", protocolId, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
/**
 * Will list NFT protocol records on chain.
 * @param {Number} count - The number of records to return.
 * @param {Number} skipFromTip - The number of transactions to skip from tip.
 * @param {Number} height - If height is not specified, it defaults to the current chain-tip.
 * @param {Boolean} regTxOnly - False for a detailed list, true for an array of transaction IDs.
 * @param {Function} callback
 */
Bitcoin.prototype.nftProtocolList = function(count, skipFromTip, height, regTxOnly, callback) {
  var self = this;
  ...
  this.client.nftProto("list", count, skipFromTip, height, regTxOnly, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
